...
fn run() {
    ...
    let mut file = std::fs::OpenOptions::new()
        .append(true)
        .open("data_50k_seed100.csv")
        .unwrap();

    file.write_all("size;duration;mean;reps\n".as_bytes())
        .expect("Unable to write data");
    st(&mut file, upper, step, reps, seed)
}

#[allow(dead_code)]
fn st(f: &mut File, upper: isize, step: usize, reps: usize, seed: isize) {
    for i in (0..upper).step_by(step) {
        let inp = (0..i)
            .map(|_| rand::thread_rng().gen_range(-seed..seed))
            .collect::<Vec<_>>();
