...
def qf() -> None:
    ...
    v.plot(qfdata, "QUnionFind", "Size", "Time in Ms", consts.QF_BENCH_CSV)
...
def wqf() -> None:
    ...
    v.plot(wqfdata, "QWUnionFind", "Size", "Time in Ms", consts.WQF_BENCH_CSV)
...
def cacheI() -> None:
    cacheI = d.load_csv(consts.CACHEI_BENCH_CSV)
    v.plot(cacheI, "CacheI", "Size", "Time in Ms", "cacheI")

def twoP() -> None:
    twoP = d.load_csv(consts.TWOP_BENCH_CSV)
